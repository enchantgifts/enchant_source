...
#if defined(NDEBUG)
#error "Enchant cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uench) are considered zero fee (for relaying and mining)
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK() || blockValue == 0 || nHeight == 2805)
        return 0;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ENCH: We use certain sporks during IBD, so check to see if they are
