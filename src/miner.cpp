...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EnchantMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("EnchantMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("EnchantMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("enchant-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running EnchantMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
