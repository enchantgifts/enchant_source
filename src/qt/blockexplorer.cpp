...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::ENCH, nValue);
...
{
    std::string genesisblockhash = "000008da6a7d3c101c3f0823a0b9dcec9a0a780a6b6f4d89eb1134e44e67f8aa";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (enchant.conf).");
            QMessageBox::warning(this, "Enchant Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
