...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    // LogPrintf("GetKernelStakeModifier(): height %d time %d interval %d\n", nStakeModifierHeight, nStakeModifierTime, nStakeModifierSelectionInterval);
...
{
    // Enchant will hash in the transaction hash and the index number in order to make sure each hash is unique
